...
function [Convergence_curve,Sorted_antlions,antlions_fitness,sorted_indexes]=ALO(Y,Max_iter,lb,ub,dim,fobj)
N = size(Y,1);
% Initialize the positions of antlions and ants
antlion_position=initialization(Y,N,dim,ub,lb);
ant_position=initialization(Y,N,dim,ub,lb);
...
antLpos=antlion_position(:,1);      %Only Column 1
antLpos=antLpos';
lb1=min(antLpos);
...
%     antlions_fitness(1,i)=fobj(antlion_position(:,1),lb); 
% end

for i=1:size(antLpos,2)
    antlions_fitness(i)=fobj(antLpos(i),lb1);
end

%Sorting the fitness function ascending order format
...
while Current_iter<Max_iter+1
    ...
    antpos=ant_position(:,1);      %Only Column 1
    antpos=antpos';
    lb2=min(antpos);
    for i=1:size(antpos,2)  
        ...
        ants_fitness(i)=fobj(antpos(i),lb2);        
%        antlions_fitness(i)=fobj(antLpos(i),lb1);
    end
